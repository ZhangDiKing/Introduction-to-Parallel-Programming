...
inline size_t numRows() { return imageInputRGBA.rows; }
inline size_t numCols() { return imageInputRGBA.cols; }
...
inline void preProcess(uchar4 **h_inputImageRGBA, uchar4 **h_outputImageRGBA,
...
inline void postProcess(const std::string& output_file, uchar4* data_ptr) {
  ...
  cv::imshow("Display window", imageOutputBGR);                   // Show our image inside it.
...
inline void generateReferenceImage(std::string input_file, std::string reference_file, int kernel_size)
{
